...
//
Функция Прочитать(НомерЛиста = 0, ПредставлениеМодели = Неопределено, НастройкаПолей = Неопределено, ОписаниеОшибок = Неопределено) Экспорт
	...
	Возврат ПредставлениеМоделиИзXDTO(
		ДанныеXDTO(ИмяВременногоФайла, НомерЛиста, ОписаниеОшибок),
		ПредставлениеМодели,
		НастройкаПолей);
		
КонецФункции

Функция ДанныеXDTO(Файл, НомерЛиста, ОписаниеОшибки = Неопределено) Экспорт
	
	ДанныеXDTO = Неопределено;
	
	Прокси = ПроксиКомпоненты(ОписаниеОшибки);
	Если Прокси <> Неопределено Тогда
		ДанныеXDTO = Прокси.parseExcel(Файл, НомерЛиста);
